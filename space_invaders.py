import pgzero, math, re, time
from random import randint
player = Actor("traffic_cone", (400, 550))
boss = Actor("star")
gameStatus = 0
highScore = []
...
def updateAliens():
    ...
    for a in range(len(aliens)):
        ...
        if randint(0, 1) == 0:
            aliens[a].image = "alien"
        else:
            aliens[a].image = "alien"
            if randint(0, 5) == 0:
                lasers.append(Actor("laser", (aliens[a].x,aliens[a].y)))
                lasers[len(lasers)-1].status = 0
...
def initBases():
    ...
    for b in range(3):
        for p in range(3):
            bases.append(Actor("fire_hydrant", midbottom=(150+(b*200)+(p*40),520)))
